...
# Read data
df <- read_excel("Fungi_Richness.xlsx")

# Force all RR and Vi columns to be numeric
df <- df %>%
  mutate(across(c(Chao1_RR, Chao1_Vi, ACE_RR, ACE_Vi, ASV_RR, ASV_Vi), as.numeric))

# Fixed-effect model combination function (automatically skips NA values)
...
write_xlsx(result, "FixedEffect_FungalRichness_Combined.xlsx")
...
# Read and convert data again for plotting
df <- read_excel("Fungi_Richness.xlsx")
...
# Determine whether CIs overlap for each Beta_ID
...
# Merge significance results back to the main long format dataframe
long_df <- long_df %>%
  left_join(significance_summary, by = "Beta_ID")

# Create forest plot: points + horizontal error bars
...
# Save plot as PDF
ggsave("Fungi_richness_forest_plot.pdf", p1, width = 8, height = 12)
